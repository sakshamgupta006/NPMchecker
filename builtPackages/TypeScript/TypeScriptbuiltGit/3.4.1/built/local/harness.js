    ...
    function createDiagnosticMessageReplacer(diagnosticMessage, replacer) {
        ...
        return function (text) {
            ...
            return text.replace(regExp, function (_) {
                ...
                return ts.formatStringFromArgs(diagnosticMessage.message, replacer.apply(void 0, [fixedArgs].concat(args)));
            });
        };
    }
...
(function (fakes) {
    ...
    function expectedDiagnosticToText(_a) {
    ...
    function compareProgramBuildInfoDiagnostic(a, b) {
    ...
    var SolutionBuilderHost = /** @class */ (function (_super) {
        ...
        function SolutionBuilderHost() {
            ...
            _this.createProgram = ts.createEmitAndSemanticDiagnosticsBuilderProgram;
            _this.diagnostics = [];
            return _this;
        }
        SolutionBuilderHost.prototype.readFile = function (path) {
            var value = _super.prototype.readFile.call(this, path);
            if (!value || !ts.isBuildInfoFile(path))
                return value;
            var buildInfo = ts.getBuildInfo(value);
            if (buildInfo.program) {
                // Fix lib signatures
                for (var _i = 0, _a = ts.getOwnKeys(buildInfo.program.fileInfos); _i < _a.length; _i++) {
                    var path_1 = _a[_i];
                    if (ts.startsWith(path_1, "/lib/")) {
                        var currentValue = buildInfo.program.fileInfos[path_1];
                        ts.Debug.assert(currentValue.signature === path_1);
                        ts.Debug.assert(currentValue.signature === currentValue.version);
                        var text = _super.prototype.readFile.call(this, path_1);
                        var signature = ts.generateDjb2Hash(text);
                        buildInfo.program.fileInfos[path_1] = { version: signature, signature: signature };
                    }
                }
            }
        ...
        SolutionBuilderHost.prototype.writeFile = function (fileName, content, writeByteOrderMark) {
            if (!ts.isBuildInfoFile(fileName))
                return _super.prototype.writeFile.call(this, fileName, content, writeByteOrderMark);
            ...
            if (buildInfo.program) {
                ...
                for (var _i = 0, _a = ts.getOwnKeys(buildInfo.program.fileInfos); _i < _a.length; _i++) {
                    ...
                    if (ts.startsWith(path, "/lib/")) {
                        var currentValue = buildInfo.program.fileInfos[path];
                        ts.Debug.assert(currentValue.signature === currentValue.version);
                ...
                if (buildInfo.program.referencedMap) {
                    var referencedMap = {};
                    for (var _b = 0, _c = ts.getOwnKeys(buildInfo.program.referencedMap).sort(); _b < _c.length; _b++) {
                        var path = _c[_b];
                ...
                if (buildInfo.program.exportedModulesMap) {
                    ...
                    for (var _d = 0, _e = ts.getOwnKeys(buildInfo.program.exportedModulesMap).sort(); _d < _e.length; _d++) {
                        var path = _e[_d];
            ...
            _super.prototype.writeFile.call(this, fileName, ts.getBuildInfoText(buildInfo), writeByteOrderMark);
        };
        ...
        SolutionBuilderHost.prototype.assertDiagnosticMessages = function () {
            ...
            var actual = this.diagnostics.slice().map(function (d) { return d.messageText; });
...
(function (Harness) {
    ...
    (function (SourceMapRecorder) {
        ...
        function getSourceMapRecordWithVFS(fs, sourceMapFile) {
            var _a;
            var sourceMapRecorder = new Harness.Compiler.WriterAggregator();
            var prevSourceFile;
            var files = ts.createMap();
            var sourceMap = ts.tryParseRawSourceMap(fs.readFileSync(sourceMapFile, "utf8"));
            if (sourceMap) {
            ...
            function getFile(path) {
                ...
                if (existing)
                    return existing;
                var value = new documents.TextDocument(path, fs.readFileSync(path, "utf8"));
                files.set(path, value);
                return value;
            }
        }
        SourceMapRecorder.getSourceMapRecordWithVFS = getSourceMapRecordWithVFS;
...
(function (Harness) {
    ...
    (function (Compiler) {
        ...
        function getDefaultLibrarySourceFile(fileName) {
            ...
            if (!libFileNameSourceFileMap) {
                libFileNameSourceFileMap = ts.createMapFromTemplate((_a = {},
                    _a[Compiler.defaultLibFileName] = createSourceFileAndAssertInvariants(Compiler.defaultLibFileName, Harness.IO.readFile(Harness.libFolder + "lib.es5.d.ts"), /*languageVersion*/ 7 /* Latest */),
                    _a));
            }
            var sourceFile = libFileNameSourceFileMap.get(fileName);
            if (!sourceFile) {
                libFileNameSourceFileMap.set(fileName, sourceFile = createSourceFileAndAssertInvariants(fileName, Harness.IO.readFile(Harness.libFolder + fileName), 7 /* Latest */));
            }
            return sourceFile;
        }
        ...
        function doTypeAndSymbolBaseline(baselinePath, program, allFiles, opts, multifile, skipTypeBaselines, skipSymbolBaselines, hasErrorBaseline) {
            ...
            var fullWalker = new TypeWriterWalker(program, /*fullTypeCheck*/ true, !!hasErrorBaseline);
...
(function (ts) {
    ...
    (function (TestFSWithWatch) {
        ...
        function checkArray(caption, actual, expected) {
            checkMapKeys(caption, ts.arrayToMap(actual, ts.identity), expected);
            ...
            for (var _i = 0, expected_1 = expected; _i < expected_1.length; _i++) {
                var f = expected_1[_i];
                assert.isTrue(ts.contains(actual, f), caption + ": expected to find " + f + " in " + actual);
            }
        }
...
(function (FourSlash) {
    ...
    var TestState = /** @class */ (function () {
        ...
        TestState.prototype.symbolsInScope = function (range) {
            ...
            return this.getChecker().getSymbolsInScope(node, 67220415 /* Value */ | 67897832 /* Type */ | 1920 /* Namespace */);
        };
        ...
        TestState.prototype.baselineGetEmitOutput = function () {
            ...
            Harness.Baseline.runBaseline(ts.Debug.assertDefined(this.testData.globalOptions["BaselineFile" /* baselineFile */]), resultString);
        };
        TestState.prototype.baselineQuickInfo = function () {
            var _this = this;
            var baselineFile = this.testData.globalOptions["BaselineFile" /* baselineFile */] ||
                ts.getBaseFileName(this.activeFile.fileName).replace(".ts" /* Ts */, ".baseline");
            Harness.Baseline.runBaseline(baselineFile, stringify(this.testData.markers.map(function (marker) { return ({
                marker: marker,
        ...
        TestState.prototype.checkPostEditInvariants = function () {
            ...
            var referenceSourceFile = ts.createLanguageServiceSourceFile(this.activeFile.fileName, createScriptSnapShot(content), 7 /* Latest */, /*version:*/ "0", /*setNodeParents:*/ false);
        ...
        TestState.prototype.verifyNewContent = function (_a, changes) {
            ...
            if (newRangeContent !== undefined) {
                ...
                var _loop_9 = function (newFileName) {
        ...
        TestState.prototype.generateTypes = function (examples) {
            for (var _i = 0, examples_1 = examples; _i < examples_1.length; _i++) {
                ...
                if (outputBaseline) {
                    if (actual === undefined)
                        throw ts.Debug.fail();
                    Harness.Baseline.runBaseline(ts.combinePaths("generateTypes", outputBaseline + ".d.ts" /* Dts */), actual);
                }
                else {
    ...
    function runFourSlashTestContent(basePath, testType, content, fileName) {
        ...
        var output = ts.transpileModule(content, { reportDiagnostics: true, compilerOptions: { target: 2 /* ES2015 */ } });
...
(function (FourSlashInterface) {
    ...
    var Verify = /** @class */ (function (_super) {
        ...
        Verify.prototype.generateTypes = function () {
            var options = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                options[_i] = arguments[_i];
            }
    ...
    (function (Completion) {
        ...
        var varEntry = function (name) { return ({ name: name, kind: "var", kindModifiers: "declare" }); };
        ...
        function globalTypesPlus(plus) {
            ...
            ].concat(globalTypeDecls, plus, Completion.typeKeywords);
        }
        ...
        // TODO: many of these are inappropriate to always provide
        Completion.globalsInJsInsideFunction = function (plus) { return [
            { name: "arguments", kind: "local var" },
            { name: "globalThis", kind: "module" }
        ].concat(Completion.globalsVars, plus, [
            { name: "undefined", kind: "var" }
        ...
        Completion.globals = [
            { name: "globalThis", kind: "module" }
        ].concat(Completion.globalsVars, [
            { name: "undefined", kind: "var" }
        ], Completion.globalKeywords);
        Completion.globalsInJs = [
            { name: "globalThis", kind: "module" }
        ].concat(Completion.globalsVars, [
            { name: "undefined", kind: "var" }
