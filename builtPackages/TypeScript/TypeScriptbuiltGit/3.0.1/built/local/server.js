    ...
    (function (server) {
        ...
        var ProjectService = (function () {
            ...
            ProjectService.prototype.stopWatchingConfigFilesForClosedScriptInfo = function (info) {
                var _this = this;
                ts.Debug.assert(!info.isScriptOpen());
                this.forEachConfigFileLocation(info, true, function (configFileName, canonicalConfigFilePath) {
            ...
            ProjectService.prototype.forEachConfigFileLocation = function (info, infoShouldBeOpen, action) {
                var _this = this;
                if (this.syntaxOnly) {
                    return undefined;
                }
                ts.Debug.assert(!infoShouldBeOpen || this.openFiles.has(info.path));
            ...
            ProjectService.prototype.getConfigFileNameForFile = function (info, infoShouldBeOpen) {
                var _this = this;
                if (infoShouldBeOpen)
                    ts.Debug.assert(info.isScriptOpen());
                this.logger.info("Search path: " + ts.getDirectoryPath(info.fileName));
                var configFileName = this.forEachConfigFileLocation(info, infoShouldBeOpen, function (configFileName, canonicalConfigFilePath) {
            ...
            ProjectService.prototype.sendConfigFileDiagEvent = function (project, triggerFile) {
                ...
                this.eventHandler({
                    ...
                    data: { configFileName: project.getConfigFilePath(), diagnostics: project.getAllProjectErrors(), triggerFile: triggerFile }
                });
            };
            ...
            ProjectService.prototype.reloadConfiguredProjectForFiles = function (openFiles, delayReload, shouldReloadProjectFor) {
                ...
                openFiles.forEach(function (openFileValue, path) {
                    ...
                    var info = _this.getScriptInfoForPath(path);
                    ts.Debug.assert(info.isScriptOpen());
                    var configFileName = _this.getConfigFileNameForFile(info, true);
                    if (configFileName) {
            ...
            ProjectService.prototype.getProjectForFileWithoutOpening = function (fileName) {
                var scriptInfo = this.filenameToScriptInfo.get(fileName) ||
                    this.getOrCreateScriptInfoNotOpenedByClientForNormalizedPath(fileName, this.currentDirectory, undefined, undefined, undefined);
                if (!scriptInfo)
                    return undefined;
                if (scriptInfo.containingProjects.length) {
                    return { scriptInfo: scriptInfo, projects: scriptInfo.containingProjects };
                }
                var configFileName = this.getConfigFileNameForFile(scriptInfo, false);
                var project = configFileName === undefined ? undefined : this.findConfiguredProjectByProjectName(configFileName) || this.createConfiguredProject(configFileName);
                return project && project.containsScriptInfo(scriptInfo) ? { scriptInfo: scriptInfo, projects: [project] } : undefined;
            };
            ...
            ProjectService.prototype.openClientFileWithNormalizedPath = function (fileName, fileContent, scriptKind, hasMixedContent, projectRootPath) {
                ...
                if (!project && !this.syntaxOnly) {
                    configFileName = this.getConfigFileNameForFile(info, true);
                    if (configFileName) {
...
(function (ts) {
    ...
    (function (server) {
        ...
        function combineProjectOutput(defaultValue, getValue, projects, action, comparer, areEqual) {
            var outputs = ts.flatMap(isProjectsArray(projects) ? projects : projects.projects, function (project) { return action(project, defaultValue); });
        ...
        function callbackProjectAndLocation(projectAndLocation, projectService, toDo, seenProjects, cb) {
            ...
            cb(projectAndLocation, function (project, location) {
                ...
                if (!originalLocation)
                    return false;
                var originalProjectAndScriptInfo = projectService.getProjectForFileWithoutOpening(server.toNormalizedPath(originalLocation.fileName));
        ...
        var Session = (function () {
            ...
            Session.prototype.getCompileOnSaveAffectedFileList = function (args) {
                var _this = this;
                var info = this.projectService.getScriptInfoEnsuringProjectsUptoDate(args.file);
                if (!info) {
                    return server.emptyArray;
                }
                var projects = args.projectFileName ? [this.projectService.findProject(args.projectFileName)] : info.containingProjects;
                var symLinkedProjects = !args.projectFileName && this.projectService.getSymlinkedProjects(info);
                return combineProjectOutput(info, function (path) { return _this.projectService.getScriptInfoForPath(path); }, symLinkedProjects ? { projects: projects, symLinkedProjects: symLinkedProjects } : projects, function (project, info) {
                    var result;
